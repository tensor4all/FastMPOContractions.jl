...
function Base.show(io::IO, obj::AdaptiveTCILeaf{C}) where {C}
  ...
  return println(
    io, "  "^length(prefix) * "Leaf $(prefix): rank=$(maximum(_linkdims(obj.data)))"
  )
end
...
struct AdaptiveTCIInternalNode{C} <: AbstractAdaptiveTCINode{C}
  ...
  function AdaptiveTCIInternalNode{C}(
    children::Dict{Int,AbstractAdaptiveTCINode{C}}, prefix::Vector{Int}
  ) where {C}
...
"""
function AdaptiveTCIInternalNode{C}(
  children::Vector{AbstractAdaptiveTCINode{C}}, prefix::Vector{Int}
) where {C}
...
"""
function _to_tree(
  patches::Dict{Vector{Int},C}; nprefix=0
