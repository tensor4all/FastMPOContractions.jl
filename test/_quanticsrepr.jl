...
"""
function fused_quantics_to_qubit(
    indices::AbstractVector{QuanticsInd{N}},
)::Vector{QubitInd} where {N}
...
"""
function qubit_to_fused_quantics(
    ::Val{D},
    indices::AbstractVector{QubitInd},
)::Vector{QuanticsInd{D}} where {D}
    indices_ = reshape(indices, D, :)
    nquantics = size(indices_, 2)
    return collect(QuanticsInd{D}(Tuple(indices_[:, n])) for n = 1:nquantics)
end
...
function fused_quantics_to_index(inds::AbstractVector{QuanticsInd{D}}) where {D}
    ...
    return Tuple(qubit_to_index(qubits[n, :]) for n = 1:D)
end
...
function index_to_fused_quantics(indices::NTuple{D,Int}, R::Int) where {D}
    ...
    return qubit_to_fused_quantics(
        Val(D),
...
function qubit_to_index(::Val{D}, inds::AbstractVector{QubitInd}, R::Int) where {D}
    ...
    return NTuple{D,Int}(qubit_to_index(inds[i:D:end]) for i = 1:D)
end
